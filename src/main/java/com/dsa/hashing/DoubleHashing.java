...
public class DoubleHashing {
    ...
    private int secondAsciiHashFunction(String word, int M) {
        int sum, idx;
        for (sum = 0, idx = 0; idx < word.length(); idx++) {
            sum = sum + word.charAt(idx);
        }
        while (sum > 15)
            sum = addAllNumbers(sum);
        return sum % M;
    }

    private int addAllNumbers(int val) {
        int remainder = 0;
        while (val > 0) {
            remainder = val % 10;
            val = val / 10;
    ...
    public void insertIntoHashTable(String word) {
        ...
        if (loadFactor > 0.75) {
            ...
            int index1 = secondAsciiHashFunction(word, hashtable.length);
            if (hashtable[index] == null) {
                hashtable[index] = word;
            } else {
                int counter = 0;
                for (int idx = index; idx < index + hashtable.length; idx++) {
                    int newIndex = (index + idx * index1) % hashtable.length;
